...
本项目以用户提供的 Mudi7 原版英文资源为基准，英文与中文词典均为 825 个键，顺序完全一致。安装包包含中文词典和一份共享字体子集，所有普通、粗体、等宽及中文动态文字角色均指向这一份字体，覆盖固定界面、短信正文、运营商名称、3500 个现代汉语高频字以及 `config/extra-glyphs.txt` 中可扩展的 SSID 白名单；保留小屏 TrueType hinting，同时删除横向界面不使用的竖排度量和 OpenType 布局表，避免打包完整大字体。另处理词典以外的显示点：
