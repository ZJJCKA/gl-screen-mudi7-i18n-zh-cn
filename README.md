...
本项目以用户提供的 Mudi7 原版英文资源为基准，英文与中文词典均为 825 个键，顺序完全一致。安装包包含中文词典和一份共享的静态界面字形子集；动态中文内容由设备原装 `default_cn_medium.ttf` 后备显示，避免重复加载和打包完整大字体。精简字库不专门收录中文 SSID，按定稿使用英文或数字 SSID。另处理词典以外的显示点：
